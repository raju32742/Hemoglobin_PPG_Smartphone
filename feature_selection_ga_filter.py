...
#        self.get_final_scores(pop,fits)
        ...
        print("Best fitness from all generations: " +str(np.max(self.best_fitness)))
        print("Mean fitness from all generations: " +str(np.mean(self.mean_fitness)))
        
        return best_ind
#        return pop
    
    def plot_feature_set_score(self,ngen, L_nam=""):
#        print(len(self.mean_fitness))
#        print(len(self.best_fitness))
        
        plt.figure(figsize=(8,6))
#        plt.title("Feature Set Fitness")
        plt.plot(self.best_fitness, 'r', linewidth=2, marker='o', 
                 label="Best Fitness("+str("%0.3f" % np.max(self.best_fitness))+")")
        plt.plot(self.mean_fitness, 'k', linewidth=2, marker='o', 
                 label="Mean Fitness("+str("%0.3f" % np.max(self.mean_fitness))+")")
#        plt.xlabel("Generation")
#        plt.ylabel("Fitness value")
        plt.xlim(0, len(self.best_fitness))
        plt.legend()
        plt.tight_layout()
        plt.savefig("imgs/"+ str(L_nam)+".png", dpi = 100)
        plt.show()
